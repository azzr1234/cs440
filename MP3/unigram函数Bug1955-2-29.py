...
def unigram_likelihood(train_set, train_labels, dev_set, unigram_smoothing_parameter, pos_prior):
    ...
    for i in range(0,len(dev_set)):
        ...
        for i in Dev_Counter.items():
            string=i[0]
            frequency=i[1]
            Positive_Likelihood+=math.log10((Postive_Counter.get(string,0)+unigram_smoothing_parameter)/(N_positive+unigram_smoothing_parameter))
            Negative_Likelihood+=math.log10((Negative_Counter.get(string,0)+unigram_smoothing_parameter)/(N_negative+unigram_smoothing_parameter))
...
def naiveBayesMixture(train_set, train_labels, dev_set, bigram_lambda,unigram_smoothing_parameter, bigram_smoothing_parameter, pos_prior):
    ...
    unigram_positive,unigram_negative= unigram_likelihood(train_set, train_labels, dev_set, unigram_smoothing_parameter, pos_prior)
    print('unigram results',unigram_positive,unigram_negative)
    #generating bigram of the text with nltk.bigrams
    for i in range(0,len(train_labels)):
        if train_labels[i]==1:#positive label
            each_review_positive=train_set[i]
            Postive_Counter+=Counter(nltk.bigrams(each_review_positive))
            len_positive+=1
        if train_labels[i]==0:#negative label
            each_review_negative=train_set[i]
            Negative_Counter+=Counter(nltk.bigrams(each_review_negative))
            len_negative+=1
    N_positive=sum(Postive_Counter.values())#follow the notation in MP3 document
    N_negative=sum(Negative_Counter.values())
    for i in range(0,len(dev_set)):
        ...
        for i in Dev_Counter.items():
            string=i[0]
            frequency=i[1]
            Positive_Likelihood+=math.log10((Postive_Counter.get(string,0)+bigram_smoothing_parameter)/(N_positive+bigram_smoothing_parameter*frequency))
            Negative_Likelihood+=math.log10((Negative_Counter.get(string,0)+bigram_smoothing_parameter)/(N_negative+bigram_smoothing_parameter*frequency))
